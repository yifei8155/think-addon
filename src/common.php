...
// 插件目录
define('ADDON_PATH', APP_PATH.'common'.DS.'addons'.DS);
...
function get_addon_autoload_config($truncate = false)
{
    ...
    foreach ($addons as $name => $addon) {
        ...
        // 读取出所有公共方法
        $methods = (array)get_class_methods("app\\common\\addons\\" . $name . "\\" . ucfirst($name));
        // 跟插件基类方法做比对，得到差异结果
...
function get_addon_class($name, $type = 'hook', $class = null)
{
    ...
    switch ($type) {
        case 'controller':
            $namespace = "\\app\\common\\addons\\" . $name . "\\controller\\" . $class;
            break;
        default:
            $namespace = "\\app\\common\\addons\\" . $name . "\\" . $class;
    }
