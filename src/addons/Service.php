...
class Service
{
     ...
     * @return  boolean
     */
    public static function importsql($name, $EM=true)
    {
        $sqlFile = ADDON_PATH . $name . DS . ($EM?'install.sql':'uninstall.sql');
...
//         }
    ...
    public static function install($name, $force = false, $extend = [])
    {
        ...
        if (is_dir($sourceAssetsDir)) {
            getCopyDir($sourceAssetsDir, $destAssetsDir);
        }
        foreach (self::getCheckDirs() as $k => $dir) {
            if (is_dir($addonDir . $dir)) {
                getCopyDir($addonDir . $dir, ROOT_PATH . $dir);
            }
        }
    ...
    public static function uninstall($name, $force = false)
    {
        ...
        // 导入
        Service::importsql($name, false);

        // 移除插件目录
        getDelDir(ADDON_PATH.$name);
    ...
    {
        return ADDON_PATH . $name . DS . 'addons' . DS;
    }
